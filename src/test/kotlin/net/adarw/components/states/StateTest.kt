...
class StateTest {
    ...
    fun `HookState should compute internalId correctly`() {
        val state = HookStateDefinition(entityId = "light.living_room", dataType = StateType.INT)
    ...
    fun `HookState should throw exception for invalid entityId`() {
        assertThrows<IllegalArgumentException> {
            HookStateDefinition(entityId = "invalid-id", dataType = StateType.STRING)
        }
    }

    @Test
    fun `InternalState should compute internalId correctly`() {
        val state = InternalStateDefinition(id = "custom_id", dataType = StateType.BOOLEAN)
    ...
    fun `HookState should serialize and deserialize correctly`() {
        val state: StateDefinition = HookStateDefinition(entityId = "switch.kitchen", dataType = StateType.BOOLEAN)
        val json = format.encodeToString(state)
        println(json)
        assertTrue(json.contains(""""type": "hook""""))

        val decoded = format.decodeFromString<StateDefinition>(json)
        assertEquals(state, decoded)
    }

    @Test
    fun `InternalState should serialize and deserialize correctly`() {
        val state: StateDefinition = InternalStateDefinition(id = "local_01", dataType = StateType.STRING)
        val json = format.encodeToString(state)
        println(json)
