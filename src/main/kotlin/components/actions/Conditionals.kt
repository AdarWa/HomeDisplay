    ...
    init {
        require(
            state.dataType == StateType.INT ||
                ...
                state.dataType == StateType.STRING ||
                state.dataType == StateType.BOOLEAN
        ) {
        ...
        require(
            (state.dataType == StateType.INT && value is StateValue.IntValue) ||
                (state.dataType == StateType.FLOAT &&
                    value is StateValue.FloatValue) ||
                (state.dataType == StateType.STRING &&
                    value is StateValue.StringValue) ||
                (state.dataType == StateType.BOOLEAN &&
                    value is StateValue.BooleanValue)
        ) {
    ...
    override fun evaluate(): Boolean {
        ...
        return when (current) {
            is StateValue.IntValue ->
                compare(current.data, (value as StateValue.IntValue).data)
            is StateValue.FloatValue ->
                compare(current.data, (value as StateValue.FloatValue).data)
            is StateValue.StringValue ->
                compare(current.data, (value as StateValue.StringValue).data)
            is StateValue.BooleanValue ->
                compare(current.data, (value as StateValue.BooleanValue).data)
        }
    }

    private fun <T : Comparable<T>> compare(left: T, right: T): Boolean =
        when (operator) {
...
data class StateRangeCondition(
    ...
    init {
        ...
        require(min != null || max != null) {
            "Range condition requires min or max"
        }
        require(
            min == null ||
                (state.dataType == StateType.INT &&
    ...
    override fun evaluate(): Boolean {
        ...
        return when (current) {
            is StateValue.IntValue -> {
                checkBounds(
                    current.data,
                    min as StateValue.IntValue?,
                    max as StateValue.IntValue?,
                )
            }
            ...
            is StateValue.FloatValue -> {
                checkBounds(
                    current.data,
                    min as StateValue.FloatValue?,
                    max as StateValue.FloatValue?,
                )
            }
    ...
    private fun <T : Comparable<T>> checkBounds(
        value: T,
        minValue: StateValue?,
        maxValue: StateValue?,
    ): Boolean {
        ...
        if (minComparable != null) {
            val minOk =
                if (inclusive) value >= minComparable else value > minComparable
            if (!minOk) return false
        }
        if (maxComparable != null) {
            val maxOk =
                if (inclusive) value <= maxComparable else value < maxComparable
...
data class ConditionalAction(
    ...
    init {
        require(!(ifTrue == null && ifFalse == null)) {
            "Both actions of a ConditionalAction cannot be null"
        }
    }
