...
fun main() {
    ...
    val annotatedClasses = reflections.getTypesAnnotatedWith(ProtoGenerate::class.java)
    ...
    val descriptors = annotatedClasses.map { clazz ->
        try {
            serializer(clazz).descriptor
        } catch (e: Exception) {
            throw RuntimeException("Failed to get serializer for ${clazz.name}. Is it marked with @Serializable?", e)
        }
    }
    ...
    println("Included ${annotatedClasses.size} messages: ${annotatedClasses.joinToString { it.simpleName }}")
}